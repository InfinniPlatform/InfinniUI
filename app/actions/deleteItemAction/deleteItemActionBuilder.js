...
_.extend(DeleteItemActionBuilder.prototype, {
    build: function(context, args){
        var dataSource = args.parentView.getContext().dataSources[args.metadata.Items.Source];
        var accept = (args.metadata['Accept'] === false) ? false: true;

        if(!dataSource){
            console.log("%c %s: не найден DataSource с именем %s", "color: red", args.parentView.getName(), args.metadata.Items.Source);
        }
        ...
        action.setProperty('propertyName', args.metadata.Items.Property);
